...
class Item implements \Suin\RSSWriter\ItemInterface
{
	...
	{
		$this->enclosures[] = array($url, $length, $type);
		return $this;
	}
	...
	public function asXML()
	{
		...
		$xml->addChild('wfw:wfw:commentRSS', $this->comments);
		$xml->addChildCData('description', $this->description);
		...
		{
	      if (is_array($enclosure) && (count($enclosure) == 3))
			{
				$element = $xml->addChild('enclosure');
	            $element->addAttribute('url',  $enclosure[0]);
	            $element->addAttribute('type', $enclosure[2]);
	            
	            if ($enclosure[1]) 
	            {
	                $element->addAttribute('length', $enclosure[1]);
	            }
			}
		}
